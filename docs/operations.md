...
`atlas-algo-sort-scope` is centered on algorithms work in OCaml. The technical target is: Package an OCaml local lab for sort analysis with node-edge fixtures, cycle and reachability reports, and documented operating limits.
...
The score is deterministic. The current threshold is 150. Risk has a penalty of 6, latency has a penalty of 2, and weight contributes a bonus of 6.
